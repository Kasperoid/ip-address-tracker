...
export const fetcher = async (url: string, { arg }: { arg: string }): Promise<ipAddressType | undefined> => {
  ...
  if (!resp.data.success) {
    ...
    if (isErrorApi(resp.data)) {
      ...
      error = createError(UNKNOWN_ERROR.message, UNKNOWN_ERROR.code, UNKNOWN_ERROR.name)
    }
    throw error
  }
  return resp.data
